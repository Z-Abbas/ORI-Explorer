...
def calculateScore(X, y, model, folds):
    ...
    ROCArea = roc_auc_score(y, y_probs)
    print('AUC: ',ROCArea)
    ...
    return {'sn' : sensitivity, 'sp' : specificity, 'acc' : accuracy, 'MCC' : MCC, 'AUC' : ROCArea, 
            'precision' : precision, 'F1' : F1Score, 'fpr' : fpr, 'tpr' : tpr, 'thresholds' : thresholds, 'lossValue' : lossValue}
...
for i, (train_index, test_index) in enumerate(kf.split(data_io,labels)): #New Try
        ...
        predicted_y = model_CBC.predict(selected_X_test)
        
        print(metrics.classification_report(y_test, predicted_y))
        print(metrics.confusion_matrix(y_test, predicted_y))
       
        testing_result.append(calculateScore(selected_X_test, y_test, model_CBC, folds));
            
temp_dict = (testing_result)
OutputDir = '/home/zeeshan/ORI/Results/BN/'
