...
def calculateScore(X, y, model, folds):
    ...
    ROCArea = roc_auc_score(y, pred_y)    #Old Method
...
optimization = Nadam(learning_rate=0.00019007194589958247)
...
def model_cnn():
    ...
    x1 = Conv1D(128, 1, activation='relu',padding="same")(inputs)
    x1=Dropout(0.2)(x1)

    x2 = Conv1D(64, 3, activation='relu',padding="same")(inputs) 
    x2=Dropout(0.2)(x2)
    
    x4 = Conv1D(16, 5, activation='relu',padding="same")(inputs) 
    x4=Dropout(0.2)(x4)
    
    x3 = Conv1D(96, 7, activation='relu',padding="same")(inputs)
    x3=Dropout(0.2)(x3)
    
    x5 = Conv1D(128, 9, activation='relu',padding="same")(inputs)
    x5=Dropout(0.2)(x5)
    
    x6 = Conv1D(128, 11, activation='relu',padding="same")(inputs)
    x6=Dropout(0.2)(x6)
    ...
    lstm_out3 = Bidirectional(GRU(64, return_sequences=True), merge_mode='mul')(conv_out)
    ...
    a1 = Dense(96, activation='relu',bias_regularizer = regularizers.l2(1e-3))(a1) # Number of nodes in hidden layer
    
    a2 = Dense(96, activation='relu',bias_regularizer = regularizers.l2(1e-3))(a1)
...
for i, (train_index, test_index) in enumerate(kf.split(data_io,labels)): #New Try
        ...
        model.save('/home/zeeshan/ORI/Results/ATxg/model'+str(i+1)+'.h5')
...
final_acc = np.mean(testing_result)  
print('Acc = ', final_acc)       
